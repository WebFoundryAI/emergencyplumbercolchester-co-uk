...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Colchester gets higher priority)
    const isColchester = location.slug === 'colchester';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isColchester ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isColchester ? 1 : 2 + i),
    });
